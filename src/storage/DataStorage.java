...
public class DataStorage {
    ...
    static {
        try {
            ...
            System.err.println("Gagal memuat MySQL JDBC Driver: " + e.getMessage());
        }
    }
    ...
    private void initializeDatabase() {
        ...
        String username = "windah_basudara";
        String password = "windah123";
        ...
        try (Connection conn = DriverManager.getConnection(url, username, password);
            ...
            stmt.executeUpdate("INSERT IGNORE INTO books (book_id, title, author, publication_year, type, subject, is_borrowed) VALUES " +
            ...
            stmt.executeUpdate("INSERT IGNORE INTO books (book_id, title, author, publication_year, type, url, subject, is_borrowed) VALUES " +
            ...
            stmt.executeUpdate("INSERT IGNORE INTO users (username, password, role, full_name, class_name, nim) VALUES " +
            ...
            System.out.println("Database inisialisasi berhasil (MySQL) dengan pengguna windah_basudara.");
        } catch (SQLException e) {
            System.err.println("Gagal menginisialisasi database dengan windah_basudara: " + e.getMessage());
            try {
                String altUsername = "zhafira";
                String altPassword = "zhafira123";
                try (Connection conn = DriverManager.getConnection(url, altUsername, altPassword);
                     Statement stmt = conn.createStatement()) {
                    stmt.execute("CREATE TABLE IF NOT EXISTS books (" +
                            "book_id VARCHAR(50) PRIMARY KEY, " +
                            "title VARCHAR(200) NOT NULL, " +
                    ...
                    stmt.executeUpdate("INSERT IGNORE INTO users (username, password, role, full_name, class_name, nim) VALUES " +
                            ...
                            "('userb', 'userb123', 'user', 'Budi Santoso', 'SI 2023 B', '2023002'), " +
                            "('windah_basudara', 'windah123', 'user', 'Windah Basudara', 'TI 2023 A', '2023006')");
                    System.out.println("Database inisialisasi berhasil dengan pengguna zhafira sebagai fallback.");
                }
            } catch (SQLException e2) {
                System.err.println("Gagal menginisialisasi database dengan zhafira: " + e2.getMessage());
                e2.printStackTrace();
            }
            e.printStackTrace();
        }
    ...
    public synchronized void loadDataFromDatabase() {
        ...
        try (Connection conn = DriverManager.getConnection(url, username, password);
            ...
            System.err.println("Gagal memuat data dengan windah_basudara: " + e.getMessage());
            try {
                String altUsername = "zhafira";
                String altPassword = "zhafira123";
                try (Connection conn = DriverManager.getConnection(url, altUsername, altPassword);
                    ...
                    System.out.println("Data dimuat dengan zhafira: " + bookCount + " buku, " + loans.size() + " peminjaman, " + users.size() + " pengguna.");
                }
            } catch (SQLException e2) {
                System.err.println("Gagal memuat data dengan zhafira: " + e2.getMessage());
                e2.printStackTrace();
            }
            e.printStackTrace();
        }
    ...
    public synchronized void loadFinesFromDatabase() {
        ...
        try (Connection conn = DriverManager.getConnection(url, username, password);
            ...
            System.out.println("Memuat denda dari database (MySQL) ke DataStorage: " + fines.size() + " denda ditemukan.");
        } catch (SQLException e) {
            System.err.println("Gagal memuat denda dari database (MySQL) ke DataStorage: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    public synchronized void saveFineToDatabase(Fine fine) {
        ...
        try (Connection conn = DriverManager.getConnection(url, username, password);
             PreparedStatement pstmt = conn.prepareStatement(
                     "INSERT INTO fines (loan_id, book_title, book_type, borrower_name, class_name, nim, loan_date, days_late, fine_amount) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) " +
                     "ON DUPLICATE KEY UPDATE book_title = VALUES(book_title), book_type = VALUES(book_type), borrower_name = VALUES(borrower_name), " +
                     "class_name = VALUES(class_name), nim = VALUES(nim), loan_date = VALUES(loan_date), days_late = VALUES(days_late), fine_amount = VALUES(fine_amount)")) {
            ...
            pstmt.executeUpdate();
            System.out.println("Denda untuk peminjaman " + fine.getLoanId() + " disimpan/diperbarui di database melalui DataStorage (MySQL): " + fine.getFineAmount());
            ...
            System.err.println("Gagal menyimpan denda ke database melalui DataStorage (MySQL): " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    public synchronized void deleteFineFromDatabase(String loanId) {
        ...
        try (Connection conn = DriverManager.getConnection(url, username, password);
            ...
            if (rowsAffected > 0) {
                ...
                System.out.println("Denda untuk peminjaman " + loanId + " dihapus dari database melalui DataStorage (MySQL): " + rowsAffected + " baris dihapus.");
            }
        } catch (SQLException e) {
            System.err.println("Gagal menghapus denda dari database melalui DataStorage (MySQL): " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    public synchronized void saveBook(Book book) {
        ...
        try (Connection conn = DriverManager.getConnection(url, username, password);
             PreparedStatement pstmt = conn.prepareStatement(
                     "INSERT INTO books (book_id, title, author, publication_year, type, url, subject, is_borrowed) VALUES (?, ?, ?, ?, ?, ?, ?, ?) " +
                     "ON DUPLICATE KEY UPDATE title = VALUES(title), author = VALUES(author), publication_year = VALUES(publication_year), " +
                     "type = VALUES(type), url = VALUES(url), subject = VALUES(subject), is_borrowed = VALUES(is_borrowed)")) {
            ...
            System.err.println("Gagal menyimpan buku dengan windah_basudara: " + e.getMessage());
            try {
                String altUsername = "zhafira";
                String altPassword = "zhafira123";
                try (Connection conn = DriverManager.getConnection(url, altUsername, altPassword);
                    ...
                    pstmt.setBoolean(8, book.isBorrowed());
                    pstmt.executeUpdate();
                    System.out.println("Buku disimpan dengan zhafira: " + book.getTitle());
                }
            } catch (SQLException e2) {
                System.err.println("Gagal menyimpan buku dengan zhafira: " + e2.getMessage());
                e2.printStackTrace();
            }
            e.printStackTrace();
        }
    ...
    public synchronized void saveLoan(Loan loan) {
        ...
        try (Connection conn = DriverManager.getConnection(url, username, password);
             PreparedStatement pstmt = conn.prepareStatement(
                     "INSERT INTO loans (loan_id, book_id, borrower_name, class_name, nim, loan_date, return_date, is_returned) VALUES (?, ?, ?, ?, ?, ?, ?, ?) " +
                     "ON DUPLICATE KEY UPDATE loan_date = VALUES(loan_date), return_date = VALUES(return_date), is_returned = VALUES(is_returned)");
            ...
            System.err.println("Gagal menyimpan peminjaman dengan windah_basudara: " + e.getMessage());
            try {
                String altUsername = "zhafira";
                String altPassword = "zhafira123";
                try (Connection conn = DriverManager.getConnection(url, altUsername, altPassword);
                    ...
                    pstmtBook.setString(2, loan.getBook().getId());
                    pstmtBook.executeUpdate();
                    System.out.println("Peminjaman disimpan dengan zhafira: " + loan.getLoanId());
                }
            } catch (SQLException e2) {
                System.err.println("Gagal menyimpan peminjaman dengan zhafira: " + e2.getMessage());
                e2.printStackTrace();
            }
            e.printStackTrace();
    ...
    public synchronized void updateLoan(Loan loan) {
        ...
        try (Connection conn = DriverManager.getConnection(url, username, password);
            ...
            System.err.println("Gagal memperbarui peminjaman dengan windah_basudara: " + e.getMessage());
            try {
                String altUsername = "zhafira";
                String altPassword = "zhafira123";
                try (Connection conn = DriverManager.getConnection(url, altUsername, altPassword);
                    ...
                    pstmt.setString(3, loan.getLoanId().trim());
                    int rowsUpdated = pstmt.executeUpdate();
                    System.out.println("Peminjaman diperbarui dengan zhafira: " + loan.getLoanId() + " (Rows affected: " + rowsUpdated + ")");
                ...
                System.err.println("Gagal memperbarui peminjaman dengan zhafira: " + e2.getMessage());
                e2.printStackTrace();
            }
            e.printStackTrace();
        }
    ...
    public synchronized boolean deleteLoan(String loanId) {
        ...
        try (Connection conn = DriverManager.getConnection(url, username, password);
            ...
            System.err.println("Gagal menghapus peminjaman dengan windah_basudara: " + e.getMessage());
            try {
                String altUsername = "zhafira";
                String altPassword = "zhafira123";
                try (Connection conn = DriverManager.getConnection(url, altUsername, altPassword);
                    ...
                    if (rowsAffected > 0) {
                        loans.removeIf(loan -> loan.getLoanId().equals(loanId));
                        System.out.println("Peminjaman dihapus dari database dengan zhafira: " + loanId + " (Rows affected: " + rowsAffected + ")");
                        return true;
                    }
                }
            } catch (SQLException e2) {
                System.err.println("Gagal menghapus peminjaman dengan zhafira: " + e2.getMessage());
                e2.printStackTrace();
            }
            e.printStackTrace();
            return false;
    ...
    public synchronized void saveUser(String username, String password, String role, String fullName, String className, String nim) {
        String url = "jdbc:mysql://localhost:3306/library_db?useSSL=false";
        String usernameDb = "windah_basudara";
        String passwordDb = "windah123";
        ...
        try (Connection conn = DriverManager.getConnection(url, usernameDb, passwordDb);
            ...
            System.err.println("Gagal menyimpan user dengan windah_basudara: " + e.getMessage());
            try {
                String altUsername = "zhafira";
                String altPassword = "zhafira123";
                try (Connection conn = DriverManager.getConnection(url, altUsername, altPassword);
                    ...
                    try (PreparedStatement pstmt = conn.prepareStatement(
                        ...
                        int rowsAffected = pstmt.executeUpdate();
                        System.out.println("User " + username + " disimpan ke database dengan zhafira. Baris terpengaruh: " + rowsAffected);
                    }
                }
            } catch (SQLException e2) {
                System.err.println("Gagal menyimpan user dengan zhafira: " + e2.getMessage());
                e2.printStackTrace();
            }
            e.printStackTrace();
        }
    ...
    public synchronized boolean deleteUser(String username) {
        ...
        try (Connection conn = DriverManager.getConnection(url, usernameDb, passwordDb);
            ...
            System.err.println("Gagal menghapus user dengan windah_basudara: " + e.getMessage());
            try {
                String altUsername = "zhafira";
                String altPassword = "zhafira123";
                try (Connection conn = DriverManager.getConnection(url, altUsername, altPassword);
                    ...
                    if (rowsAffected > 0) {
                        users.removeIf(user -> user.getUsername().equals(username));
                        loadDataFromDatabase(); // Memuat ulang data untuk memastikan konsistensi
                        System.out.println("User dengan username " + username + " dihapus dari database dengan zhafira (Rows affected: " + rowsAffected + ")");
                        return true;
                    }
                }
            } catch (SQLException e2) {
                System.err.println("Gagal menghapus user dengan zhafira: " + e2.getMessage());
                e2.printStackTrace();
            }
            e.printStackTrace();
            return false;
    ...
    public synchronized boolean deleteBook(String bookId) {
        ...
        try (Connection conn = DriverManager.getConnection(url, username, password);
            ...
            System.err.println("Gagal menghapus buku dengan windah_basudara: " + e.getMessage());
            try {
                String altUsername = "zhafira";
                String altPassword = "zhafira123";
                try (Connection conn = DriverManager.getConnection(url, altUsername, altPassword);
                    ...
                    if (rowsAffected > 0) {
                        books.removeIf(book -> book.getId().equals(bookId));
                        System.out.println("Buku dengan ID " + bookId + " dihapus dari database dengan zhafira (Rows affected: " + rowsAffected + ")");
                        return true;
                    }
                }
            } catch (SQLException e2) {
                System.err.println("Gagal menghapus buku dengan zhafira: " + e2.getMessage());
                e2.printStackTrace();
            }
            e.printStackTrace();
            return false;
    ...
    public List<String[]> getAllUsersData() {
        ...
        try (Connection conn = DriverManager.getConnection(url, username, password);
            ...
            System.err.println("Gagal memuat data user dengan windah_basudara: " + e.getMessage());
            try {
                String altUsername = "zhafira";
                String altPassword = "zhafira123";
                try (Connection conn = DriverManager.getConnection(url, altUsername, altPassword);
                ...
                System.err.println("Gagal memuat data user dengan zhafira: " + e2.getMessage());
                e2.printStackTrace();
            }
            e.printStackTrace();
        }
    ...
    public String[] getUserData(String username) {
        ...
        try (Connection conn = DriverManager.getConnection(url, usernameDb, passwordDb);
            ...
            System.err.println("Gagal mengambil data user dengan windah_basudara: " + e.getMessage());
            try {
                String altUsername = "zhafira";
                String altPassword = "zhafira123";
                try (Connection conn = DriverManager.getConnection(url, altUsername, altPassword);
                ...
                System.err.println("Gagal mengambil data user dengan zhafira: " + e2.getMessage());
                e2.printStackTrace();
            }
            e.printStackTrace();
        }
    ...
    public boolean isUserExists(String username) {
        ...
        try (Connection conn = DriverManager.getConnection(url, usernameDb, passwordDb);
            ...
            System.err.println("Gagal memeriksa user dengan windah_basudara: " + e.getMessage());
            try {
                String altUsername = "zhafira";
                String altPassword = "zhafira123";
                try (Connection conn = DriverManager.getConnection(url, altUsername, altPassword);
                ...
                System.err.println("Gagal memeriksa user dengan zhafira: " + e2.getMessage());
                e2.printStackTrace();
            }
            e.printStackTrace();
            return false;
    ...
    public boolean authenticateUser(String username, String password) {
        ...
        try (Connection conn = DriverManager.getConnection(url, usernameDb, passwordDb);
            ...
            System.err.println("Gagal autentikasi user dengan windah_basudara: " + e.getMessage());
            try {
                String altUsername = "zhafira";
                String altPassword = "zhafira123";
                try (Connection conn = DriverManager.getConnection(url, altUsername, altPassword);
                ...
                System.err.println("Gagal autentikasi user dengan zhafira: " + e2.getMessage());
                e2.printStackTrace();
            }
            e.printStackTrace();
            return false;
    ...
    public String getUserRole(String username) {
        ...
        try (Connection conn = DriverManager.getConnection(url, usernameDb, passwordDb);
            ...
            System.err.println("Gagal mengambil role user dengan windah_basudara: " + e.getMessage());
            try {
                String altUsername = "zhafira";
                String altPassword = "zhafira123";
                try (Connection conn = DriverManager.getConnection(url, altUsername, altPassword);
                ...
                System.err.println("Gagal mengambil role user dengan zhafira: " + e2.getMessage());
                e2.printStackTrace();
            }
            e.printStackTrace();
            return "user";
    ...
    public User getUserByUsername(String username) {
        ...
        try (Connection conn = DriverManager.getConnection(url, usernameDb, passwordDb);
            ...
            System.err.println("Gagal mengambil user oleh username dengan windah_basudara: " + e.getMessage());
            try {
                String altUsername = "zhafira";
                String altPassword = "zhafira123";
                try (Connection conn = DriverManager.getConnection(url, altUsername, altPassword);
                ...
                System.err.println("Gagal mengambil user oleh username dengan zhafira: " + e2.getMessage());
                e2.printStackTrace();
            }
            e.printStackTrace();
        }
        ...
        try (Connection conn = DriverManager.getConnection(url, username, password)) {
            ...
            try (PreparedStatement pstmt = conn.prepareStatement(
                ...
                try {
                    String altUsername = "zhafira";
                    String altPassword = "zhafira123";
                    try (Connection connAlt = DriverManager.getConnection(url, altUsername, altPassword)) {
                        connAlt.setAutoCommit(false);
                        try (PreparedStatement pstmt = connAlt.prepareStatement(
                                "INSERT INTO books (book_id, title, author, publication_year, type, url, subject, is_borrowed) VALUES (?, ?, ?, ?, ?, ?, ?, ?) " +
                                "ON DUPLICATE KEY UPDATE title = VALUES(title), author = VALUES(author), publication_year = VALUES(publication_year), " +
                                "type = VALUES(type), url = VALUES(url), subject = VALUES(subject), is_borrowed = VALUES(is_borrowed)")) {
                            for (Book book : books) {
                            ...
                            pstmt.executeBatch();
                            connAlt.commit();
                            System.out.println("Semua buku disimpan ke database dengan zhafira.");
                        }
                    }
                } catch (SQLException e2) {
                    System.err.println("Gagal menyimpan buku dengan zhafira: " + e2.getMessage());
            ...
            System.err.println("Gagal terhubung ke database untuk menyimpan buku dengan windah_basudara: " + e.getMessage());
            e.printStackTrace();
        }
    }
}