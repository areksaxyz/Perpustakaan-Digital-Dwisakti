...
public class DigitalCatalogPanel extends JPanel {
    ...
    public DigitalCatalogPanel(DataStorage dataStorage, CardLayout cardLayout, JPanel cardPanel) {
        ...
        // Tabel katalog digital dengan kolom "No", "ID", "Judul", "Penulis", "Tahun Terbit", "URL", "Baca"
        String[] columns = {"No", "ID", "Judul", "Penulis", "Tahun Terbit", "URL", "Baca"};
        tableModel = new DefaultTableModel(columns, 0) {
            @Override
            public boolean isCellEditable(int row, int column) {
                return column == 6; // Kolom "Baca" yang dapat diedit
            }
        };
        ...
        // Kolom Baca
        table.getColumnModel().getColumn(6).setCellRenderer((table, value, isSelected, hasFocus, row, column) -> {
        ...
        table.getColumnModel().getColumn(6).setCellEditor(new DefaultCellEditor(new JCheckBox()) {
            ...
            public Component getTableCellEditorComponent(JTable table, Object value, boolean isSelected, int row, int column) {
                ...
                button.addActionListener(e -> {
                    ...
                    String bookId = (String) table.getValueAt(row, 1); // Kolom ID
                    Book book = dataStorage.getBooks().stream()
                            .filter(b -> b.getId().equals(bookId))
                            .findFirst()
                            .orElse(null);
                    if (book != null && "Digital".equals(book.getType()) && !book.getUrl().isEmpty()) {
                        JDialog loadingDialog = new JDialog((Frame) SwingUtilities.getWindowAncestor(DigitalCatalogPanel.this), "Memuat...", false);
                        loadingDialog.setLayout(new BorderLayout());
                        JLabel loadingLabel = new JLabel("Memuat dokumen, harap tunggu...", SwingConstants.CENTER);
                        loadingDialog.add(loadingLabel, BorderLayout.CENTER);
                        loadingDialog.setSize(300, 100);
                        loadingDialog.setLocationRelativeTo(DigitalCatalogPanel.this);
                        ...
                        new Thread(() -> {
                            ...
                            try {
                                ...
                                SwingUtilities.invokeLater(() -> {
                                    JOptionPane.showMessageDialog(DigitalCatalogPanel.this, "Gagal membuka atau merender PDF: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
                        ...
                        JOptionPane.showMessageDialog(DigitalCatalogPanel.this, "URL tidak tersedia atau buku bukan digital!", "Error", JOptionPane.ERROR_MESSAGE);
                    }
                });
                return button;
            }
    ...
    private void updateTable() {
        ...
        List<Book> digitalBooks = dataStorage.getBooks().stream()
                ...
                .collect(Collectors.toList()); // Gunakan Collectors.toList() untuk Java 8+
